...
export type NumberExample = {
  ...
  zpaGroup: number;          // e.g., group index [0..]
  // Labels
...
export function extractFeaturesForNumber(
  ...
  const freqFortnight = (arr: Draw[]) => {
    const endIdx = arr.length - 1;
    let c = 0;
    for (let t = Math.max(0, endIdx - 6 + 1); t <= endIdx; t++) {
      const d = arr[t];
      if (d.main.includes(n) || d.supp.includes(n)) c++;
    }
    return c;
  ...
  const freqMonth = (arr: Draw[]) => {
    const endIdx = arr.length - 1;
    let c = 0;
    for (let t = Math.max(0, endIdx - 12 + 1); t <= endIdx; t++) {
      const d = arr[t];
      if (d.main.includes(n) || d.supp.includes(n)) c++;
    }
    return c;
  ...
  const freqQuarter = (arr: Draw[]) => {
    const endIdx = arr.length - 1;
    let c = 0;
    for (let t = Math.max(0, endIdx - 36 + 1); t <= endIdx; t++) {
      const d = arr[t];
      if (d.main.includes(n) || d.supp.includes(n)) c++;
    }
    return c;
