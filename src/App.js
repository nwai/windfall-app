...
function DraggableResizableRotatableOverlay({
  ...
  function onPointerDown(e) {
    if (e.target.dataset.handle === "resize") {
      setResizing(true);
      setLastMouse({ x: e.clientX, y: e.clientY, startRadius: radius });
  ...
  function onPointerMove(e) {
    ...
    if (resizing && lastMouse) {
      ...
      setRadius(Math.max(minRadius, Math.min(maxRadius, dist)));
    }
  ...
  }, [dragging, resizing, rotating, lastMouse, pos, radius, rotation]);

  // X size should fit inside the circle
  const crossLen = radius * 1.5;
  const handleSize = 10;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <path
          d={`M 0 ${-radius}
            A ${radius} ${radius} 0 0 1 ${rotateHandle.x} ${rotateHandle.y + handleSize * 2}`}
          fill="none"
          stroke="red"
          strokeWidth={1}
...
export default function App() {
  ...
  function handleSimulateSubmit(e) {
    ...
    const newFlat = flattenDraws([newDraw]);
    setFlatData((prev) => [...prev, ...newFlat]);
  ...
  return (
    <div style={{ fontFamily: "sans-serif", margin: 20, height: "100vh", position: "relative" }}>
      <h2>
        Lotto Scatter Plot — colored balls &amp; draggable, resizable, rotatable overlay
      </h2>
      ...
      >
        <strong>Simulate a draw for next blank Friday:</strong>
      ...
      <div style={{ marginTop: 16, color: "#888" }}>
        <small>
          ...
          <b>Resize:</b> grab the small white handle at lower right.<br/>
          <b>Rotate:</b> grab the small white handle at top.
        </small>
      </div>
    </div>
  );
