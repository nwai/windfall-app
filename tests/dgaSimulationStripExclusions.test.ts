...
describe("DGA simulation strip active exclusion locks", () => {
  it("feeds active hard exclusions into both DGA simulation strips", () => {
    const calls = APP_SOURCE.match(/<DGASimulateStrip[\s\S]*?\/>/g) ?? [];

    expect(calls.length).toBeGreaterThanOrEqual(2);
    expect(calls.filter((call) => call.includes("excludedNumbers={selectionUnavailableNumbers}")).length).toBeGreaterThanOrEqual(2);
  });
  ...
  it("keeps excluded DGA strip numbers out of simulation state", () => {
    ...
    expect(APP_SOURCE).toContain("const sorted = removeUserExcludedNumbers(normalizeDgaSelectedNumbers(nums), selectionUnavailableNumbers);");
